---
name: kuaishou-sentiment-monitor
description: 快手舆情监测助手。针对任意关键词获取快手作品数据、详情、互动数据、评论数据，基于快手实时数据，综合分析舆情健康、情感占比、负面信息、事件时间、热度排行、竞品口碑对比与风险建议等，进行分析并输出舆情报告。支持三大能力：(1) 关键词搜索视频，可按点赞数、发布时间、视频时长筛选排序；(2) 达人作品抓取，按主页链接获取公开作品列表；(3) 视频评论分析，按视频链接获取评论内容与互动数据。
license: MIT
version: 1.0.0
metadata:
  type: command
...
> ✨ **一句话价值**：针对任意关键词获取快手作品数据、详情、互动数据、评论数据，基于快手实时数据，综合分析舆情健康、情感占比、负面信息、事件时间、热度排行、竞品口碑对比与风险建议等，进行分析并输出舆情报告。支持关键词搜索排序、达人作品获取、视频评论获取；用于快手、KuaiShou或Kwai 内容分析、市场调研、达人筛选、舆情监测、爆品、趋势、市场容量、竞争结构等数据形成经营判断时使用，依据快手真实返回的数据形成分析结论。
...
> - 实用: 日志自动保存到 logs 目录，适配营销报告 / 内容策划场景
...
> **Note:** 请先通过 [快手作品批量下载技能](https://www.guaikei.com) 开通TOKEN，配置环境变量 `GUAIKEI_API_TOKEN` 后才能正常运行。
...
- 官网：[快手作品研究SKILL](https://www.guaikei.com)
